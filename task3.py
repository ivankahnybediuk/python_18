...
follow the link: https://www.geeksforgeeks.org/wc-command-linux-examples/ or in case you have macOS or Linux - just call
manual for this utility via command: `man wc`).
...
count_chars(name) function that reads an input file and counts the number of characters in it (hint: file.read() returns
a single string)
...
ambitious, you may be able to improve this by passing an open file object into the two count functions (hint: file.seek(0) is a file rewind).
"""

import task3_module

if __name__ == "__main__":
    print(task3_module.charsCounter("myfile.txt"))
    print(task3_module.linesCounter("myfile.txt"))